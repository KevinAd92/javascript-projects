...
function reverseCharacters(reverseStr){
   ...
   } else if (typeof reverseStr === "object") {
    reverseStr = reverseStr.join("").reverse();
   }
   else {
    reverseStr = reverseStr.split("").reverse().join("");
   //if (reverseCharacters === typeof(Number)) 
   }

return reverseStr;
}
console.log(reverseCharacters(985758934));
...
function reverseArr(reverseStr){
    let newArr = [];
    for (let i = 0; i < reverseStr.length; i++){
   let reverseElement = reverseCharacters(reverseStr[i]);
    newArr.push(reverseElement);
}
    return newArr;
}
